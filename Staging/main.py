...
os.environ["OPENSEARCH_USER"] = os.getenv("STAGING_OPENSEARCH_USER")
os.environ["OPENSEARCH_PWD"] = os.getenv("STAGING_OPENSEARCH_PWD")
os.environ["OPENSEARCH_HOST"] = os.getenv("STAGING_OPENSEARCH_HOST")
app = Flask(__name__)
...
def create_and_return_foodhak(user_id):
    ...
    selected_url = ""  # Default value to avoid UnboundLocalError
...
@app.route('/generate2/foodhak', methods=['POST'])
...
def health_check():
    return {"status": "healthy", "message": "Foodhak Service is up and running."}
